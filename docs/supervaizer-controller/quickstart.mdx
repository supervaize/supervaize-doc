...
[<i className="bi bi-github"></i> GitHub Repository](https://github.com/runwaize/supervaize_hello_world)

```bash title="Clone the repo"
git clone https://github.com/runwaize/supervaize_hello_world.git
...
https://github.com/runwaize/supervaize_hello_world/blob/main/supervaizer_control.py#L1-L20
```
...
```
  ...
  <TabItem value="local" label="Local mode (no Studio)">

```bash
# Run your agents locally without Studio credentials
supervaizer start --local
```

:::tip
Local mode runs your agents alongside a built-in Hello World agent — no Studio account needed. Set `SUPERVAIZER_DISABLE_HELLO_WORLD=true` to hide the Hello World agent. API key defaults to `local-dev`.
:::
